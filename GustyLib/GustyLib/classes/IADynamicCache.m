...
- (void)setObject:(id)obj forKey:(id)key{
	//NSLog(@"IADynamicCache setObject: %p forKey: %@", obj, key);
...
- (id)purgeableObjectForKey:(id)key{
	IAPurgeableObject *pObj = ((IAPurgeableObject*)[self objectForKey:key]);
	return pObj.obj;
}

- (void)setPurgeableObject:(id)obj forKey:(id)key{
	IAPurgeableObject *pObj = [[[IAPurgeableObject alloc] initWithObject:obj] autorelease];
	//NSLog(@"IADynamicCache.setPurgeableObject: %p", pObj);
	[self setObject:pObj forKey:key];
}

- (BOOL)retainPurgeableObjectForKey:(id)key{
	IAPurgeableObject *pObj = [self objectForKey:key];
	if (pObj==NULL) {
		return NO;
	}else {
...
//	NSLog(@"IADynamicCache willEvictObject: %p", obj);
}
