...
@implementation IAPurgeableObject


- (id)initWithObject:(id)anObj{
    if ((self = [super init])) {
		//NSLog(@"*** IAPurgeableObject.initWithObject: %p", self);
		self.obj = anObj;
...
//	NSLog(@"IAPurgeableObject.beginContentAccess: %p", self);
...
- (void)endContentAccess{
	//NSLog(@"IAPurgeableObject.endContentAccess: %p", self);
...
- (void)discardContentIfPossible{
	//NSLog(@"IAPurgeableObject.discardContentIfPossible: %p", self);
	if (![self isContentDiscarded] && contentAccessCount==0) {
...
- (BOOL)isContentDiscarded{
	//NSLog(@"IAPurgeableObject.isContentDiscarded: %p", self);
