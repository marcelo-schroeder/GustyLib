...
@interface IAPersistenceManager ()
...
@property (strong) NSMutableDictionary *p_managedObjectChangedValuesDictionary;
@property (strong) NSMutableDictionary *p_managedObjectCommittedValuesDictionary;
@property (strong) IAEntityConfig *entityConfig;
@property (strong) NSDictionary *metadata;
@property (strong) NSMutableArray *childManagedObjectContexts;

@end

@implementation IAPersistenceManager
...
- (id)init{
    ...
    if (self) {
        ...
        self.childManagedObjectContexts = [NSMutableArray new];
    }
    return self;
}
...
-(NSMutableDictionary*)userInfoForEntityName:(NSString*)a_entityName entityUserInfo:(NSMutableDictionary*)a_entityUserInfoDict{
    if (![a_entityUserInfoDict valueForKey:a_entityName]) {
        ...
        [l_userInfoDict setValue:[[NSMutableSet alloc] init] forKey:IA_KEY_INSERTED_OBJECTS];
        [l_userInfoDict setValue:[[NSMutableSet alloc] init] forKey:IA_KEY_UPDATED_OBJECTS];
        [l_userInfoDict setValue:[[NSMutableSet alloc] init] forKey:IA_KEY_DELETED_OBJECTS];
        [l_userInfoDict setValue:[[NSMutableDictionary alloc] init] forKey:IA_KEY_UPDATED_PROPERTIES];
        [l_userInfoDict setValue:[[NSMutableDictionary alloc] init] forKey:IA_KEY_ORIGINAL_PROPERTIES];
...
//        NSLog(@"self.p_managedObjectCommittedValuesDictionary: %@", [self.p_managedObjectCommittedValuesDictionary description]);
        ...
        for (NSManagedObject *l_managedObject in [[aNotification userInfo] valueForKey:NSInsertedObjectsKey]) {
            ...
            NSString *l_entityName = [l_managedObject entityName];
            ...
            [((NSMutableSet*)[l_userInfoDict valueForKey:IA_KEY_INSERTED_OBJECTS]) addObject:l_managedObject];
...
//            NSLog(@"Notifying for %@", l_entityName);
            ...
            for (NSManagedObject *l_managedObject in [l_userInfoDict objectForKey:IA_KEY_DELETED_OBJECTS]) {
...
//                    self.p_isPrivateQueueManagedObjectContextStale = YES;
            ...
            if (!l_causeDataToGoStaleForEntity) {
                ...
                for (NSManagedObject *l_managedObject in [l_userInfoDict objectForKey:IA_KEY_INSERTED_OBJECTS]) {
                    l_causeDataToGoStaleForEntity = [self.entityConfig shouldTriggerChangeNotificationForManagedObject:l_managedObject];
                    if (l_causeDataToGoStaleForEntity) {
...
//                        self.p_isPrivateQueueManagedObjectContextStale = YES;
                ...
                if (!l_causeDataToGoStaleForEntity) {
                    ...
                    NSDictionary *l_updatedPropertiesDict = [l_userInfoDict objectForKey:IA_KEY_UPDATED_PROPERTIES];
...
//                                self.p_isPrivateQueueManagedObjectContextStale = YES;
            ...
            if (l_causeDataToGoStaleForEntity) {
                ...
                NSNotification *notification = [NSNotification notificationWithName:IA_NOTIFICATION_PERSISTENT_ENTITY_CHANGE object:NSClassFromString(l_entityName) userInfo:l_userInfoDict];
...
- (BOOL) validateUniqueKeysForManagedObject:(NSManagedObject *)aManagedObject error:(NSError**)anError{
	...
	for(NSArray* uniqueKeyArray in uniqueKeyArrays){
		...
		if (count==1) {	// it it finds only 1 it's ok because it's the managed object itself
			...
			NSString *labelForKeys = [aManagedObject m_labelForKeys:[keysAndValues allKeys]];
			if ([keysAndValues count]>1) {
			...
			NSError *detailError = [self newErrorWithCode:IA_ERROR_PERSISTENCE_DUPLICATE_KEY
											  errorMessage:errorMessage];
			if (ok) {
				ok = NO;
				if(anError!=NULL){
					*anError = [IAUtils newErrorContainerWithError:detailError];
					errorCreated = YES;
				}
			}else if (errorCreated) {
				[IAUtils addError:detailError toContainer:*anError];
			}
		}
	}
	return ok;
...
//        NSLog(@"l_propertyName: %@", l_propertyName);
        NSRelationshipDescription *l_relationshipDescription = [[[NSEntityDescription entityForName:[a_managedObject entityName] inManagedObjectContext:[self currentManagedObjectContext]] relationshipsByName] valueForKey:l_propertyName];
...
//                        NSLog(@"l_propertyLabel");
                ...
                default:
					[IAUtils handleUnrecoverableError:error];
                    break;
            }
            
		}else if([error domain]==IA_ERROR_DOMAIN_COMMON){ 
...
//    NSLog(@"Handling core data error: %@", [anErrorContainer description]);
	...
	[IAUIUtils showAlertWithMessage:message title:anAlertTitle?anAlertTitle:@"Validation Error" delegate:anAlertDelegate];
}
...
//    NSLog(@"findAllForNonSystemEntity for %@: %@", entityName, [l_array description]);
    return l_array;
}
...
- (BOOL)saveObject:(NSManagedObject *)aManagedObject{
	...
	if([self validateForSave:aManagedObject]){
		...
		if ([self.entityConfig listReorderAllowedForObject:aManagedObject]) {
			NSArray* all = [self findAllForEntity:[aManagedObject entityName] includePendingChanges:YES];
...
- (NSMutableArray *) findAllForEntity:(NSString *)entityName includePendingChanges:(BOOL)a_includePendingChanges includeSubentities:(BOOL)a_includeSubentities{
	if ([self isSystemEntityForEntity:entityName]) {
		//NSLog(@"system entity: %@", entityName);
		return [self findAllForSystemEntity:entityName];
	}else {
		//NSLog(@"ANY entity: %@", entityName);
...
- (void) setMetadata:(NSDictionary*)aMetadataDictionary{
	[[self persistentStore] setMetadata:aMetadataDictionary];
}

- (id) metadataValueForKey:(NSString *)aKey{
	return [self.metadata valueForKey:aKey];
}

- (void) setMetadataValue:(id)aValue forKey:(NSString *)aKey{
	NSMutableDictionary *mutableMetadataDictionary = [self.metadata mutableCopy];
	[mutableMetadataDictionary setValue:aValue forKey:aKey];
	[self setMetadata:mutableMetadataDictionary];
}

- (NSPersistentStore*) persistentStore{
	return [self.persistentStoreCoordinator persistentStoreForURL:self.storeUrl];
}
...
//            NSLog(@"keyPath: %@", keyPath);
            ...
            Class l_propertyClass = [IAUtils m_classForPropertyNamed:keyPath inClassNamed:anEntityName];
...
- (void)configureWithDatabaseResourceName:(NSString*)a_databaseFileName managedObjectModelResourceName:(NSString*)a_managedObjectModelResourceName{
    ...
    self.storeUrl = [NSURL fileURLWithPath:[[self applicationDocumentsDirectory] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.sqlite", a_databaseFileName]]];
    
    //	#ifdef DEBUG
    //	[self resetTestDatabase:storeUrl];
    //	#endif
...
- (void)manageDatabaseVersioningChangeWithBlock:(void (^)(NSUInteger a_oldSystemEntitiesVersion, NSUInteger a_newSystemEntitiesVersion))a_block{
    ...
    NSDictionary *l_systemEntityConfig = [IAUtils getPlistAsDictionary:@"SystemEntityConfig"];
    ...
    [IAUtils logBooleanWithLabel:@"System tables loaded" value:[self systemDbTablesLoaded]];
...
- (BOOL)validateForSave:(NSManagedObject *)aManagedObject{
	...
	if (coreDataValidationOk) {
		...
		if ([aManagedObject m_validateForSave:&error]) {
...
-(void)pushChildManagedObjectContext{
    ...
    [self.childManagedObjectContexts addObject:l_moc];
}

-(void)popChildManagedObjectContext{
    [self.childManagedObjectContexts removeLastObject];
}
...
//    NSLog(@"  from threadDictionary: %@", [l_managedObjectContext description]);
    if (!l_managedObjectContext) {
        if (self.childManagedObjectContexts.count>0) {
            l_managedObjectContext = [self.childManagedObjectContexts lastObject];
...
+ (BOOL)setValidationError:(NSError**)anError withMessage:(NSString*)anErrorMessage{
	if (anError!=NULL) {
		*anError = [IAUtils newErrorWithCode:IA_ERROR_PERSISTENCE_VALIDATION errorMessage:anErrorMessage];
		return NO;
	}else {
		return YES;
	}
