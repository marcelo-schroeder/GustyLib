...
+(NSString*)m_pListPathForName:(NSString*)a_name bundle:(NSBundle*)a_bundle{
...
+ (NSArray*) getPlistAsArray:(NSString *)pListName bundle:(NSBundle*)a_bundle{
	NSString *plistPath = [self m_pListPathForName:pListName bundle:a_bundle];
...
//wip: review the dependency on Crashlytics below
...
+(NSString*)appNameAndEdition{
    ...
    NSString *l_edition = [IAUtils appEdition];
    if (l_edition) {
...
+(NSString*)appVersionAndBuildNumber{
    return [NSString stringWithFormat:@"%@ (build %@)", [self appVersion], [IAUtils appBuildNumber]];
}

+(NSString*)appFullName{
    return [NSString stringWithFormat:@"%@ %@", [self appNameAndEdition], [IAUtils appVersionAndBuildNumber]];
}
