...
typedef NS_ENUM(NSUInteger, ECViewWidthLayout) {
...
typedef NS_ENUM(NSUInteger, ECSide) {
  /** Left side of screen */
  ECLeft,
...
typedef NS_ENUM(NSUInteger, ECResetStrategy) {
...
/** IFA_ECSlidingViewController is a view controller container that presents its child view controllers in two layers. The top layer can be panned to reveal the layers below it. */
@interface IFA_ECSlidingViewController : IFAViewController {
...
/** Convience method for getting access to the IFA_ECSlidingViewController sharedInstance */
