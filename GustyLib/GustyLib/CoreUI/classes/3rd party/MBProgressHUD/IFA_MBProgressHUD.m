...
@implementation IFA_MBProgressHUD
...
- (void)setMode:(MBProgressHUDMode)newMode {
	...
	if ([NSThread isMainThread]) {
		...
		[self performSelectorOnMainThread:@selector(setModeInternal:) withObject:l_modeNumber waitUntilDone:NO];
	}
}
...
- (void)setLabelTextInternal:(NSString *)newText {
//	if ([NSThread isMainThread]) {
...
- (void)setLabelText:(NSString *)newText {
	if ([NSThread isMainThread]) {
        [self setLabelTextInternal:newText];
	} else {
		[self performSelectorOnMainThread:@selector(setLabelTextInternal:) withObject:newText waitUntilDone:NO];
	}
}
...
- (void)setDetailsLabelTextInternal:(NSString *)newText {
//	if ([NSThread isMainThread]) {
...
- (void)setDetailsLabelText:(NSString *)newText {
	if ([NSThread isMainThread]) {
        [self setDetailsLabelTextInternal:newText];
	} else {
		[self performSelectorOnMainThread:@selector(setDetailsLabelTextInternal:) withObject:newText waitUntilDone:NO];
	}
}
...
- (void)setProgressInternal:(NSNumber*)l_progressNumber {
...
- (void)setProgress:(float)newProgress {
    NSNumber *l_progressNumber = [NSNumber numberWithFloat:newProgress];
    if ([NSThread isMainThread]) {
        [self setProgressInternal:l_progressNumber];
    } else {
        [self performSelectorOnMainThread:@selector(setProgressInternal:) withObject:l_progressNumber waitUntilDone:NO];
    }
}
...
+ (IFA_MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	IFA_MBProgressHUD *hud = [[IFA_MBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
	return hud;
}
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[IFA_MBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		IFA_MBProgressHUD *HUD = (IFA_MBProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"MBProgressHUDViewIsNillException" 
					format:@"The view used in the IFA_MBProgressHUD initializer is nil."];
	}
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        label.frame = CGRectMake(0, 0, dims.width, dims.height);
...
//        label.adjustsFontSizeToFitWidth = YES;
        ...
        CGRect lFrame = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset,
                                   floorf(indFrame.origin.y + indFrame.size.height + (self.indicator?PADDING:0.0)),
                                   lWidth, lHeight);
        label.frame = lFrame;
